...
(function () {
  ...
  var defaultButtons = [{
    name: 'select',
    title: 'Выбор и перемещение объектов (V)',
    ...
    name: 'path',
    title: 'Кривые и замыкаемые линии',
  ...
  const defaultExtendedButtons = [{
    ...
    name: 'save',
    title: 'Сохранить',
    icon: `<svg id="Capa_1" width="128px" height="128px" viewBox="0 0 30 30"><path d="M24.545 0h-2.727v8.182c0 .753-.611 1.363-1.364 1.363H8.181c-.753 0-1.363-.61-1.363-1.363V0H2.727A2.727 2.727 0 0 0-.001 2.727v24.545A2.727 2.727 0 0 0 2.727 30h24.545a2.727 2.727 0 0 0 2.727-2.728V5.454zm0 27.273H5.454v-8.182a2.727 2.727 0 0 1 2.727-2.727h13.636a2.727 2.727 0 0 1 2.728 2.727z"/>
  <path d="M16.364 0h2.727v6.818h-2.727z"/></svg>`
    ...
    name: 'tooptionvalue',
    title: 'Загрузить результат в корзину',
    icon: ``
  }, {
    name: 'clear',
    ...
    try {
      ...
      (() => {
        buttons.forEach(item => {
          $(`${this.containerSelector} #toolbar .main-buttons`).append(`<button id="${item.name}" title="${item.title}">${item.icon}</button>`);
        })
        ...
        $(`${this.containerSelector} #toolbar .main-buttons button`).click(function () {
			...
			document.querySelectorAll('.sp-replacer.sp-active').forEach(spreplacer => {			
				spcontainer.classList.remove('sp-active');
			});
			*/
          let id = $(this).attr('id');
      ...
      (() => {
        extendedButtons.forEach(item => {
          $(`${this.containerSelector} #toolbar .extended-buttons`).append(`<button id="${item.name}" title="${item.title}">${item.icon}</button>`);
        })

        $(`${this.containerSelector} #toolbar .extended-buttons button`).click(function () {
          let id = $(this).attr('id');
          if (id === 'save') {
            if (window.confirm('Сохранить результат, чтобы продолжить позже?')) {
              saveInBrowser.save('canvasEditor', _self.canvas.toJSON());
            }
          } else if (id === 'clear') {
            if (window.confirm('Холст будет очищен, все изменения будут утрачены. Продолжить?')) {
              _self.canvas.clear(), saveInBrowser.remove('canvasEditor');
            }
          } else if (id === 'download') {
			

			document.getElementById('modal-custom-savings').classList.add('active');
			let customDesignFilename = document.getElementById('designMarker').value		
			/*let customDesignFilenameHolder = document.getElementById('designFile')			
			let imgTimestamp = Date.now()
			customDesignFilenameHolder.value = imgTimestamp*/
			  
            /*$('body').append(`<div class="custom-modal-container">
              <div class="custom-modal-content">
                <!--<div class="button-download" id="svg">Скачать в формате SVG</div>-->
                <div class="button-download" id="png">Скачать в формате PNG</div>
                <div class="button-download" id="jpg">Скачать в формате JPG</div>
              </div>
            </div>`)
            ...
            $(".custom-modal-container .button-download").click(function (e) {
			  ...
			  //var originalTransform = _self.canvas.viewportTransform;
			  //_self.canvas.viewportTransform = fabric.iMatrix.slice(0);
			  var originalZoom = _self.canvas.getZoom();
			  _self.applyZoom('1');
			  _self.canvas.setZoom('1');
			  ...
			  //пробуем масштабировать
			  //_self.canvas.viewportTransform = originalTransform;
			  _self.applyZoom(originalZoom);
			  _self.canvas.setZoom(originalZoom);
            })
			...
			$.ajax({ 
				type: "POST", 
				url: "build_custom_design.php", 
				data: {  
					//imgBase64: _self.canvas.toDataURL(),
					imgBase64: _self.canvas.toDataURL({format: 'jpeg'}),
					imgName: customDesignFilename
				} 
			}).done(function(o) { 
				console.log('saved');
				//console.log(o);
				//customDesignFilenameHolder.value = o
				console.log('done');
				document.getElementById('modal-custom-savings').classList.remove('active');
			...
			/*let specDesignJsonField = document.getElementById('designJson')
			specDesignJsonField.innerHTML = JSON.stringify(_self.canvas.toJSON())*/



			
            /*$(".constructor-add-to-cart").click(function (e) {
			  var originalZoom = _self.canvas.getZoom();
			  _self.applyZoom('1');
			  _self.canvas.setZoom('1');
